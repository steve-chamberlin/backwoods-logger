...
void (*Wire_user_onRequest)(void) = NULL;
void (*Wire_user_onReceive)(int) = NULL;
...
// slave tx event callback
// or after beginTransmission(address)
void Wire_send_buffer(uint8_t* data, uint8_t quantity)
{
  if(transmitting){
  // in master transmitter mode
...
// slave tx event callback
// or after beginTransmission(address)
void Wire_send_string(char* data)
{
  Wire_send_buffer((uint8_t*)data, strlen(data));
}
